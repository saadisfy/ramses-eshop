...
{{/*
Generate application port #todo: remove this
*/}}
...
{{/*
Merge environment variables from defaults and overrides
This function merges envDefaults with envOverrides, allowing parent charts to override or add env vars
If the same key exists in both, envOverrides takes precedence
*/}}
{{- define "application.envVars" -}}
{{- $envVars := dict -}}
{{- /* First, add all envDefaults */ -}}
{{- range .Values.envDefaults | default list -}}
{{- $_ := set $envVars .name .value -}}
{{- end -}}
{{- /* Then, override/add envOverrides (these take precedence) */ -}}
{{- range .Values.envOverrides | default list -}}
{{- $_ := set $envVars .name .value -}}
{{- end -}}
{{- toYaml $envVars -}}
{{- end }}
...
{{/*
Generate environment variables for ConfigMap data section
This function creates key-value pairs for ConfigMap data from merged env vars
*/}}
{{- define "application.configMapData" -}}
{{- $envVars := dict -}}
{{- /* First, add all envDefaults */ -}}
{{- range .Values.envDefaults | default list -}}
{{- $_ := set $envVars .name .value -}}
...
{{- $_ := set $envVars .name .value -}}
{{- end -}}
{{- /* Output as key-value pairs for ConfigMap */ -}}
...
{{/*
Generate shared service names with release prefix
*/}}
{{- define "application.postgresqlServiceName" -}}
{{- $global := .Values.global.eshop.sharedServices.postgresql | default dict -}}
{{- $local := .Values.sharedServices.postgresql | default dict -}}
{{- $merged := deepCopy $global | mergeOverwrite $local -}}
{{- if $merged.serviceName -}}
{{- printf "%s-%s" .Release.Name $merged.serviceName -}}
{{- else -}}
{{- printf "%s-postgresql" .Release.Name -}}
{{- end -}}
{{- end }}

{{- define "application.rabbitmqServiceName" -}}
{{- $global := .Values.global.eshop.sharedServices.rabbitmq | default dict -}}
{{- $local := .Values.sharedServices.rabbitmq | default dict -}}
{{- $merged := deepCopy $global | mergeOverwrite $local -}}
{{- if $merged.serviceName -}}
{{- printf "%s-%s" .Release.Name $merged.serviceName -}}
{{- else -}}
...
{{- define "application.redisServiceName" -}}
{{- $global := .Values.global.eshop.sharedServices.redis | default dict -}}
{{- $local := .Values.sharedServices.redis | default dict -}}
{{- $merged := deepCopy $global | mergeOverwrite $local -}}
{{- if $merged.serviceName -}}
{{- printf "%s-%s" .Release.Name $merged.serviceName -}}
{{- else -}}
